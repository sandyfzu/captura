...
/// # Errors
///
/// Returns [`XshotError::EncodingError`] if the PNG encoder fails (e.g.
...
pub fn encode_rgba_to_png(rgba: &[u8], width: u32, height: u32) -> Result<Vec<u8>, XshotError> {
    ...
    if rgba.len() != expected_len {
        return Err(XshotError::encoding_error(format!(
            "buffer length mismatch: expected {expected_len} bytes for {width}x{height} RGBA image, got {}",
            rgba.len()
        )));
    }
    ...
    let estimated = (rgba.len() / 4).max(1024);
